...
window.signupDriver = async function signupDriverFixed(){
  ...
  const values={company:$('#cc').value.trim(),name:$('#cn').value.trim(),email:$('#ce').value.trim().toLowerCase(),phone:$('#ct').value.trim(),password:$('#cp').value,license:$('#cl').value.trim(),insurance:$('#ci').value.trim(),vehicle:$('#cv').value.trim(),capacity:+$('#cap').value};
  ...
  try{
    ...
    if(!auth.error&&auth.data.user){
      accountMessage=auth.data.session
        ? 'Your driver login is ready. Sign in to upload a clear photo of your vehicle.'
        : 'Check your email to confirm your driver login, then sign in to upload a clear photo of your vehicle.';
    }else if(auth.error){
  ...
  }catch(e){btn.disabled=false;btn.textContent='JOIN RED ROCKS DD';flash('#cm',e.message||'Unable to save your application. Please try again.',true);}
};
...
window.finishRrddProfile = async function finishRrddProfile(applicationId){
  ...
  if(!vehicleFile)return flash('#finishmsg','Please upload one clear photo of your vehicle.',true);
  const btn=$('#finishProfileBtn');
  btn.disabled=true;btn.textContent='UPLOADING…';flash('#finishmsg','Uploading your vehicle photo…');
  try{
    ...
    const patch={vehicle_photo_path:vehiclePath,payment_methods:methods,cash_required:cashRequired,profile_completed_at:new Date().toISOString()};
    if(driverPath)patch.driver_photo_path=driverPath;
    const updated=await sb.from('rrdd_driver_applications').update(patch).eq('id',applicationId).eq('email',(user.email||'').toLowerCase());
    if(updated.error)throw updated.error;
    flash('#finishmsg','Profile complete. Welcome to Red Rocks Designated Driver.');
    setTimeout(()=>dashboardPage(),600);
  }catch(e){btn.disabled=false;btn.textContent='FINISH MY DRIVER PROFILE';flash('#finishmsg',e.message||'Unable to finish your profile.',true);}
};
...
window.dashboardPage = async function dashboardPageFixed(){
  ...
  if(application){
    ...
    const complete=!!(application.vehicle_photo_path&&application.profile_completed_at);
    if(!complete){
