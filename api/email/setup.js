...
export default async function handler(req, res) {
  ...
  try {
    ...
    const endpoint = `https://www.redrocksdd.com/api/email/inbound?token=${encodeURIComponent(INBOUND_TOKEN)}`;
    const hooks = await callResend('/webhooks');
    let webhook = Array.isArray(hooks?.data)
    ...
    return res.status(200).json({
      ok: true,
      domain: fullDomain,
      webhook: { id: webhook?.id || null, endpoint, status: webhook?.status || 'created' },
    });
  } catch (error) {
