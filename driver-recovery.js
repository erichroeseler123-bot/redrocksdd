...
window.rrddRecoveryLogin = async function rrddRecoveryLogin(){
  ...
  if(!login.error){finishDriverPage();return;}
...
window.saveRecoveredRrddProfile = async function saveRecoveredRrddProfile(){
  ...
  const values={name:$('#recover-name').value.trim(),company:$('#recover-company').value.trim(),phone:$('#recover-phone').value.trim(),license:$('#recover-license').value.trim(),insurance:$('#recover-insurance').value.trim(),vehicle:$('#recover-vehicle').value.trim(),capacity:+$('#recover-capacity').value};
  ...
  if(!vehicleFile)return flash('#recover-save-msg','Please upload one clear photo of your vehicle.',true);

  const dates=$$('.recover-date:checked').map(x=>x.value);
  const btn=$('#recover-save-btn');
  btn.disabled=true;btn.textContent='UPLOADING & SAVING…';flash('#recover-save-msg','Uploading your vehicle photo and finishing your profile…');
  try{
    const vehiclePath=await rrddUploadPhoto(user,vehicleFile,'vehicle');
    const driverPath=driverFile?await rrddUploadPhoto(user,driverFile,'driver'):null;
    const saved=await sb.from('rrdd_driver_applications').insert({driver_name:values.name,company_name:values.company,email:(user.email||'').trim().toLowerCase(),phone:values.phone,license_number:values.license,insurance_carrier:values.insurance||null,vehicle:values.vehicle,capacity:values.capacity,availability_dates:dates,payment_methods:methods,cash_required:cashRequired,driver_photo_path:driverPath,vehicle_photo_path:vehiclePath,profile_completed_at:new Date().toISOString(),status:'review_pending',source:'redrocksdd_driver_signup'});
    if(saved.error)throw saved.error;
    $('#app').innerHTML=`<section class="sec white"><div class="w" style="max-width:700px"><div class="card" style="text-align:center"><span class="status">PROFILE COMPLETE</span><h2>WELCOME TO RED ROCKS DESIGNATED DRIVER.</h2><p class="lead">Your driver information, vehicle photo, availability and payment preferences are saved.</p><p>We’ll finish the commercial-authority review and contact you if we need anything else.</p><p><b>Questions?</b><br>Call or text 720-369-6292<br>erichroeseler123@gmail.com</p><a class="btn dark" href="#/dashboard">OPEN DRIVER DASHBOARD</a></div></div></section>`;
  }catch(e){btn.disabled=false;btn.textContent='UPLOAD VEHICLE PHOTO & FINISH MY PROFILE';flash('#recover-save-msg',e.message||'Unable to finish your profile. Please try again or contact Erich.',true);}
};
