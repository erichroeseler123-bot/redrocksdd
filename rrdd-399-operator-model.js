...
async function rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,stage,error){
  ...
  try{
    await sb.rpc('save_driver_signup_recovery',{
      ...
      p_company_name:values.company||null,
      p_license_number:values.license||null,
      p_insurance_carrier:values.insurance||null,
      p_island_served:null,
      p_vehicle:values.vehicle||null,
      p_vehicle_capacity:values.capacity||null,
      p_driver_price_cents:null,
      p_availability:{dates:selectedDates||[]},
      p_payment_methods:paymentMethods||[],
      p_cash_required:!!cashRequired,
      p_stage:stage||'operator_form_received',
      p_last_error:error||null,
      p_source:'redrocksdd_operator_application',
      p_extra:{applicant_type:values.applicantType||'limo_company'},
      p_completed:stage==='profile_complete'
    });
  }catch(_e){}
}
...
window.companyPage=async function rrddSplitCompanyPage(){
  $('#app').innerHTML=`<section class="sec white"><div class="w">
    ...
    <div class="grid">
      ...
      <div class="card"><h3>One form. No account.</h3><p>Choose taxi operator or limo company, enter your commercial credentials, and upload one clear Chevrolet Suburban photo. Your form information is saved when you submit it, even if something later needs attention.</p></div>
    </div>
...
window.signupDriver=async function rrddSplitOneStepSignup(){
  ...
  await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'operator_form_received',null);

  if(!$('#agree')?.checked){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'validation_failed','Licensing/service confirmation missing');return flash('#cm','Please confirm the licensing and service requirements.',true)}
  if(!values.name||!values.company||!values.email||!values.phone||!values.license||!values.vehicle||!values.capacity){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'validation_failed','Required information missing');return flash('#cm','Please complete the required fields.',true)}
  if(!values.email.includes('@'))return flash('#cm','Enter a valid email address.',true);
  if(!/suburban/i.test(values.vehicle)){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'validation_failed','Vehicle is not a Chevrolet Suburban');return flash('#cm','Red Rocks DD is currently accepting Chevrolet Suburbans only.',true)}
  if(values.capacity<1||values.capacity>20){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'validation_failed','Invalid passenger capacity');return flash('#cm','Enter the number of passengers your Suburban can legally carry.',true)}
  if(!paymentMethods.length){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'validation_failed','No payment method selected');return flash('#cm',`Choose at least one way customers can pay your $${e.pay/100} operator amount.`,true)}
  let vehiclePhoto=null;try{vehiclePhoto=await fileData('#rrdd-vehicle-photo')}catch(err){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'photo_failed',err.message||'Unable to read vehicle photo');return flash('#cm',err.message||'Unable to read the vehicle photo.',true)}if(!vehiclePhoto){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'photo_missing','Vehicle photo missing');return flash('#cm','Upload one clear photo of the Chevrolet Suburban.',true)}
  ...
  if(result.error){await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'application_save_failed',result.error.message||'Final application save failed');btn.disabled=false;btn.textContent='SUBMIT MY OPERATOR APPLICATION';return flash('#cm','We saved your form information, but the final application needs attention. You do not need to start over.',true)}
  await rrddPreserveOperatorForm(values,selectedDates,paymentMethods,cashRequired,'profile_complete',null);
